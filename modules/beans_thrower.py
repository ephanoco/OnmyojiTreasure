...
class BeansThrower:
    ...
    def throw_beans(self):
        ...
        if not pt_enter:
            pt_enter = self.dict_demon_parade['enter']['pt'] = self.matcher.match(self.dict_demon_parade['enter']['path'])[0]
        self.matcher.left_click(pt_enter, (4, 5))
        pt_start = self.dict_demon_parade['start']['pt']
        if not pt_start:
            pt_start = self.dict_demon_parade['start']['pt'] = self.matcher.match(self.dict_demon_parade['start']['path'])[0]
        self.__choose_ghost(pt_start)  # Choose the ghost
        self.matcher.left_click(pt_start, (4, 5))
        # TODO
        while True:
            time.sleep(1)
            self.matcher.capture(True, 'static/temp/town/demon_parade/ghosts/')  # HACK

    def __choose_ghost(self, pt_start):
        ghosts_scatter = self.__get_ghosts_scatter(pt_start)
        self.matcher.left_click(ghosts_scatter[2], (1, 2))  # The right ghost is selected by default
    ...
    def __invite_friends(self, index: str = ''):
        ...
        if not pt_invite:
            pt_invite = self.dict_demon_parade['invite']['pt'] = \
                self.matcher.match(self.dict_demon_parade['invite']['path'])[0]
        self.matcher.left_click(pt_invite, (1, 2))
        # Invite someone from friend list
        friend_list_scatter = self.__get_friend_list_scatter()
        cur_index = 0 if not index else int(index)
        self.matcher.left_click(friend_list_scatter[cur_index], (1, 2))

    def __get_friend_list_scatter(self):
        scatter = self.dict_demon_parade['friend_list_scatter']
        if not scatter:
            cx, cy = self.matcher.match(self.dict_demon_parade['friends']['path'])[0]
